...
> Builds into `maestro/` · uses **Skill 2** and the **test matrix** (`test-cases.md`)
...
emulator at the **broken** build. You *run and triage* the whole suite in Section 13 —
...
Maestro login flows for TechShop Android (appId com.techshop.android). Read the matrix to
decide which flows to write and what each one asserts — cover exactly the login cases I
designed, no more and no less. Put a reusable valid login in maestro/subflows/login.yaml and
one flow per case under maestro/flows/.
...
If the matrix contains a case Maestro cannot assert (it can't read the masked-input
attribute or a colour), say so and skip it here — note that it belongs to Appium/Espresso.
...
masked-input or colour cases — it should flag those for Appium/Espresso rather than writing
a weak assertion.
...
the unresponsive "Proceed to Checkout" button (BUG-011 — they can only be verified on the
fixed build); respect those markings and explain the dependency in the flow.
...
```
Without changing any flow, run your login happy-path flow against the Compose build, then
reinstall the React Native build (same package id) and run the same flow again. Explain why
the identical flow works on both, and where in the hierarchy the two apps differ.
```

**Expected:** the same YAML passes on both builds — the payoff of stable resource-ids. And
because Maestro is cross-platform, these same flows carry to iOS with barely a change.
