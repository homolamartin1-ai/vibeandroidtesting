# Section 4 — Exploring TechShop Android with an AI Agent

> 📖 **Spec:** [../techshop/requirements.md](../techshop/requirements.md)

The agent drives the emulator via the mobile MCP and explores the app *before any test
exists*. You capture what it finds into `exploration-notes.md` — the raw material for the
test matrix in Section 7.

> **Explore the BROKEN build** (`techshop/compose-broken` or `techshop/reactnative-broken`) —
...
```
First, make sure the app under test is the BROKEN build — techshop/compose-broken
(or techshop/reactnative-broken), NOT the fixed build. Install a clean, fresh copy:
uninstall any existing com.techshop.android from the emulator (adb uninstall
com.techshop.android), then reinstall the broken build and launch it, so we explore the
pristine buggy version with no leftover state.
...
describe what actually happened (which screen, any error) and list the resource-ids of
every field and button you interacted with. Do not write any test yet.
```
...
elements and their resource-ids (flag any control with NO id), the flows you tried, and
anything that looked wrong or surprising versus what a login should do.
```
...
flow to exploration-notes.md — elements + resource-ids + anything that behaves oddly. If a
button does nothing when tapped, record exactly that.
```
