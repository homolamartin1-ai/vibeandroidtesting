  ...
  const renderRoom = ({ item }) => {
    ...
    return (
      <View style={styles.card} testID={`room-card-${item.id}`} accessibilityLabel={`room-card-${item.id}`}>
        {/* Image / Icon Preview with Red Available Badge (BUG-008) */}
...
const styles = StyleSheet.create({
  ...
  badge: { backgroundColor: '#ef4444', color: '#ffffff', fontSize: 12, fontWeight: 'bold', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8, overflow: 'hidden' },
